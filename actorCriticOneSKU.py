...
import numpy as np
from environment_single_sku import InventoryEnv
import time
np.random.seed(2)
...
class Actor(object):
    def __init__(self, sess, n_features,n_outputs, action_bound, lr=0.0001):
        ...
        # self.sd = tf.constant(10, dtype = tf.float32)
    ...
    def choose_action(self, s, i_episode):
        s = s[np.newaxis, :]
        a, myMu, mySD = self.sess.run([self.action, self.mu, self.sigma], {self.s: np.squeeze(s,2)})  # get probabilities for all actions
        ...
        # time.sleep(0.5)
        if i_episode > 60:
            print 'Mean: {}\tStd. Dev: {}\tAction: {}'.format(np.squeeze(myMu), np.squeeze(mySD), np.squeeze(a))
        a = np.squeeze(a,1)
        # a = a.T **WRONGGGGGG
