...
#PREVIOUS WORKING VER : environment_vectorised_operations.py
# Warehouse mapping
# Whse_A = 0
# Whse_C = 1
# Whse_J = 2
...
TODO:
	...
	DONE VECTORISE ORDER DEDUCTIONS
	DONE VECTORISE COST COMPUTATION
	CURRENTLY ONLY DEALING WITH ONE PRODUCT : SCALE TO ENTIRE 4 x 100 (LINE 102)
	
	CHANGE CPs OF ALL PRODUCTS, TO PREVENT THE REWARD BLOWUP, HENCE THE GRADIENT BLOWUP.****

CHANGES :
	DEMAND IS GAUSSIAN DISTRIBUTED NOW, WITH DATASET DEMAND AS MEAN AND CONSTANT S.D. 
'''
...
class InventoryEnv:
	...
	def order_demand_deduction(self):
		...
		# 	demand_temp = np.int(np.random.normal(demands_for_current_day.loc[i, 'Order_Demand'],self.DEMAND_STD_DEV))
	...
	def step(self, action):
		...
		self.obs_flat_date = np.hstack([self.obs_flat,np.array([self.day_of_year])])
		self.obs_flat_date = np.reshape(self.obs_flat_date, (self.obs_flat_date.shape[0],1))
		return self.obs_flat_date, np.squeeze(self.reward) / 100
